...
if (process.env.EDGE_DEBUG) {
	console.log(
		"Load edge native library from: " + edgeNative
	);
}
...
exports.func = function (language, options) {
	...
	if (options.references !== undefined) {
		if (!Array.isArray(options.references)) {
			throw new Error(
				"The references property must be an array of strings."
			);
		}
	...
	if (options.assemblyFile) {
		if (!options.typeName) {
			var matched = options.assemblyFile.match(
				/([^\\\/]+)\.dll$/i
			);
			if (!matched) {
				throw new Error(
